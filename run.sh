...
clang++ -c -O0 tests/test1.cc -emit-llvm -S -o build/test1.ll
clang++ -c tests/test1-main.cc -emit-llvm -S -o build/test1-main.ll
...
opt -load passes/LLVMPass.so -csi < build/test1.ll > /dev/null 2> build/csi.result
opt -load passes/LLVMPass.so -cdi < build/test1.ll -o build/test1-cdi.bc
opt -load passes/LLVMPass.so -bb < build/test1.ll -o build/test1-bb.bc
